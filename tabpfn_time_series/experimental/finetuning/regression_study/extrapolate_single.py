...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=-2.0,
        help="The minimum slope for the line generator.",
    )
    parser.add_argument(
        "--max_slope",
        type=float,
        default=2.0,
        help="The maximum slope for the line generator.",
    )
    parser.add_argument(
        "--min_intercept",
        type=float,
        default=-5.0,
        help="The minimum intercept for the line generator.",
    )
    parser.add_argument(
        "--max_intercept",
        type=float,
        default=5.0,
        help="The maximum intercept for the line generator.",
    )
    parser.add_argument(
    ...
    NUM_META_VAL_SAMPLES = 1  # Hold-out set for validation
    NUM_MIN_PERIODS = 9
    NUM_MAX_PERIODS = 12

    # --- Masking Configuration ---
    MASKING_STRATEGY = "future"
    # Block strategy
    MASK_START_FRAC = 0.4
    MASK_LEN_FRAC = 0.2
    ...
    output_root = Path("output/finetune_extrapolation")
    ...
    if args.dataset_generator == "sinusoid":
    ...
    elif args.dataset_generator == "line":
        meta_X_val, meta_y_val = generate_line_dataset(
            num_samples=NUM_META_VAL_SAMPLES,
            num_points=NUM_POINTS,
        ...
        raise ValueError(f"Unknown dataset generator: {args.dataset_generator}")

    meta_X_train = meta_X_val[:, : int(meta_X_val.shape[1] * (1 - FUTURE_MASK_FRAC))]
    meta_y_train = meta_y_val[:, : int(meta_y_val.shape[1] * (1 - FUTURE_MASK_FRAC))]
    ...
    print(
        "META_X_TRAIN (min, max): ",
        np.min(meta_X_train, axis=1),
        np.max(meta_X_train, axis=1),
    )
    print(
        "META_X_VAL (min, max): ",
        np.min(meta_X_val, axis=1),
        np.max(meta_X_val, axis=1),
    )
    ...
    visualize_meta_training_samples(
        ...
        num_samples_to_visualize=min(5, len(meta_X_train)),
